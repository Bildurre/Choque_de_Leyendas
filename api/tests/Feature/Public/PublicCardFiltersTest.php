...
it('devuelve subtipos, tipos de equipo, rangos y subtipos de ataque localizados', function () {
    $beast = publicCardSubtype(); // 'Bestia' / 'Beast'
    $aura = publicCardSubtype(['name' => ['es' => 'Aura', 'en' => 'Aura']]);
    $sword = publicEquipmentType(); // 'Espada' / 'Sword'
    ...
    expect(collect($response->json('subtypes'))->pluck('id')->all())
        ...
        ->and($response->json('equipment_types'))->toBe([['id' => $sword->id, 'name' => 'Espada']])
...
it('localiza los nombres con ?locale', function () {
    ...
    expect($response->json('factions.0.name'))->toBe('Alliance')
        ...
        ->and($response->json('equipment_types.0.name'))->toBe('Sword')
