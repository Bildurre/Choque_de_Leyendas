...
// Formulario de rango de ataque en modal. Sin endpoint show: en edición se
// rellena desde el ítem ya cargado en el listado (prop target).
const props = defineProps<{
  modelValue: boolean
  mode: 'create' | 'edit'
  target?: AttackRange | null
}>()
...
const { create, update } = useResource<AttackRange>(api, '/admin/attack-ranges')
...
const title = computed(() =>
  props.mode === 'create' ? t('attackRanges.new') : t('attackRanges.edit'),
)
...
async function submit() {
  ...
  try {
    if (props.mode === 'edit' && props.target) {
      ...
      toast.success(t('attackRanges.toast.updated'))
    } else {
      await create({ name: form.name })
      toast.success(t('attackRanges.toast.created'))
    }
    emit('saved')
    emit('update:modelValue', false)
  } catch (e) {
    mapServerErrors(e)
    toast.danger(t('attackRanges.toast.saveError'))
  } finally {
    saving.value = false
  }
}
...
<template>
  ...
  >
    <TranslatableInput
      ...
      :label="t('attackRanges.fields.name')"
      required
      :error="errors.name"
    />
  </EditModal>
