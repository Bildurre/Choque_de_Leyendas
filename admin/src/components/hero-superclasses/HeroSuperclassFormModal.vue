...
// Formulario de superclase en modal. Sin endpoint show: en edición se
// rellena desde el ítem ya cargado en el listado (prop target).
const props = defineProps<{
  modelValue: boolean
  mode: 'create' | 'edit'
  target?: HeroSuperclass | null
}>()
...
const { create, update } = useResource<HeroSuperclass>(api, '/admin/hero-superclasses')
...
const title = computed(() =>
  props.mode === 'create' ? t('heroSuperclasses.new') : t('heroSuperclasses.edit'),
)
...
async function submit() {
  ...
  try {
    if (props.mode === 'edit' && props.target) {
      ...
      toast.success(t('heroSuperclasses.toast.updated'))
    } else {
      await create({ name: form.name, name_female: form.name_female })
      toast.success(t('heroSuperclasses.toast.created'))
    }
    emit('saved')
    emit('update:modelValue', false)
  } catch (e) {
    mapServerErrors(e)
    toast.danger(t('heroSuperclasses.toast.saveError'))
  } finally {
    saving.value = false
  }
}
...
<template>
  ...
  >
    <TranslatableInput
      ...
      :label="t('heroSuperclasses.fields.name')"
      required
      :error="errors.name"
    />
    ...
    <TranslatableInput
      ...
      :locales="locales.locales"
      :label="t('heroSuperclasses.fields.nameFemale')"
